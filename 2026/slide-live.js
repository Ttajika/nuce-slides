...
(function () {
  ...
  function resetSent() {
    sent = { i: -1, refs: [], lastLen: 0, brefs: [], blastLen: 0, boardOpen: null, ptr: null, lastFull: 0 };
  }
  ...
  //  VIEWER
  // ═══════════════════════════════════════════════════════════════════════
  const L = { i: -1, s: {}, b: {}, bo: false, p: null, ended: false, presenter: false };
  let following = true;
  ...
  function onMessage(m) {
    ...
    switch (m.t) {
      case 'full':
        ...
        applySlide(); applyBoard(); redrawLive(); redrawLiveBoard();
        setBadge();
        break;
      case 'list': {
  ...
  function injectViewerUI() {
    ...
    f.onclick = () => {
      following = !following;
      if (following) { applySlide(); applyBoard(); }
      setBadge();
    };
